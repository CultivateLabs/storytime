...
describe Storytime::Importers::Wordpress do
  ...
  describe "#import!" do
    it "imports" do
      ...
      expect(posts[1].content).to match("After you read this")
      expect(posts[1].user).to eq(user)
      expect(posts[1].created_at).to eq(Time.strptime("2011-08-16 22:00:35 UTC", "%Y-%m-%d %H:%M:%S %Z"))
      expect(posts[1].published_at).to eq(Time.strptime("Mon, 28 Jul 2014 02:42:21 +0000", "%a, %d %b %Y %H:%M:%S %z"))

      expect(posts[2].title).to eq("Hello world!")
      expect(posts[2].content).to match("After you read this")
      expect(posts[2].user).to eq(user)
      expect(posts[2].created_at).to eq(Time.strptime("2011-08-16 22:00:35 UTC", "%Y-%m-%d %H:%M:%S %Z"))
      expect(posts[2].published_at).to eq(Time.strptime("Mon, 28 Jul 2014 02:43:07 +0000", "%a, %d %b %Y %H:%M:%S %z"))

      expect(posts[3].title).to eq("Hello world!")
      expect(posts[3].content).to match("After you read this")
      expect(posts[3].user).to eq(user)
      expect(posts[3].created_at).to eq(Time.strptime("2011-08-16 22:00:35 UTC", "%Y-%m-%d %H:%M:%S %Z"))
